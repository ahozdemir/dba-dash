...
namespace DBAHawk
{
    ...
    public class DBCollector : IErrorLogger
    {
        ...
        public string ConnectionID => Data.Tables["DBAHawk"]?.Rows[0]["ConnectionID"].ToString();
        ...
        /// <summary>
        /// Minimum database compatibility level DBA-Hawk supports.  90 = SQL Server 2005, the minimum
        ...
        private void StopCollection()
        {
            ...
            LogInternalPerformanceCounter("DBAHawk", "Collection Duration (ms)", currentCollection, Convert.ToDecimal(swatch.Elapsed.TotalMilliseconds));
        }

        private async Task StartupAsync()
        {
            noWMI = Source.NoWMI;
            Data = new DataSet("DBAHawk");
        ...
        /// <summary>
        /// Add Metadata relating to the DBA Hawk service used for collection.
        /// </summary>
        ...
        public async Task GetInstanceAsync()
        {
            ...
            var dt = await GetDataTableAsync("DBAHawk", SqlStrings.Instance, CollectionCommandTimeout.GetDefaultCommandTimeout());
        ...
        private async Task CollectPlansAsync()
        {
            ...
            LogInternalPerformanceCounter("DBAHawk", "Count of plans collected", "", dt?.Rows.Count ?? 0); // Count of plans actually collected - might be less than the list of plans we wanted to collect
        }
        ...
        private List<Plan> GetPlansToCollect()
        {
            if (Data.Tables.Contains("RunningQueries"))
            {
                ...
                LogInternalPerformanceCounter("DBAHawk", "Count of plans meeting threshold for collection", "", plans.Count); // Total number of plans that meet the threshold for collection
                LogInternalPerformanceCounter("DBAHawk", "Count of plans to collect", "", collectList.Count); // Total number of plans we want to collect (plans that meet the threshold that are not cached)
                LogInternalPerformanceCounter("DBAHawk", "Count of plans from cache", "", plans.Count - collectList.Count); // Plan count we didn't collect because they have been collected previously and we cached the handles/hashes.
        ...
        private void CollectText()
        {
            ...
            LogInternalPerformanceCounter("DBAHawk", "Count of text collected", "", dt.Rows.Count); // Count of text collected from sql_handles
            LogInternalPerformanceCounter("DBAHawk", "Count of running queries", "", Data.Tables["RunningQueries"]!.Rows.Count); // Total number of running queries
        }
...
INSERT INTO @handles(sql_handle)");
            ...
            LogInternalPerformanceCounter("DBAHawk", "Distinct count of text (sql_handle)", "", handles.Count); // Total number of distinct sql_handles
            LogInternalPerformanceCounter("DBAHawk", "Count of text (sql_handle) to collect", "", cnt); // Count of sql_handles we need to collect
            LogInternalPerformanceCounter("DBAHawk", "Count of text (sql_handle) from cache", "", handles.Count - cnt); // Count of sql_handles we didn't need to collect because they were collected previously and we cached the sql_handle.
