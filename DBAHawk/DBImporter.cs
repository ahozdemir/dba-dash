...
namespace DBAHawk
{
    public class DBImporter
    {
        ...
        {
            var agentRow = data.Tables["DBAHawk"]!.Rows[0];
        ...
        public async Task UpdateAsync()
        {
            ...
            var rInstance = data.Tables["DBAHawk"]?.Rows[0];
        ...
        private static DBADashAgent GetAgent(DataRow row)
        {
            return new DBADashAgent()
            {
                ...
                AgentServiceName = row.Table.Columns.Contains("AgentServiceName") ? (string)row["AgentServiceName"] : "{DBAHawkService}",
