...
namespace DBAHawk
{
    ...
    public static class ServiceTools
    {
        ...
        private static readonly string serviceFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DBAHawkService.exe");
        ...
        /// <summary>
        /// Checks if the current service account (the account running the DBA Hawk service) has sufficient permissions to perform an upgrade.
        /// This validates that:
        ...
        public static bool CanServiceAccountPerformUpgrade(CollectionConfig config)
        {
            try
            {
                ...
                // Check if any DBA Hawk processes are running that cannot be terminated
                if (!CanTerminateRunningProcesses())
                {
                    Log.Warning("Service account does not have permissions to terminate one or more running DBA Hawk processes");
                    return false;
                }
        ...
        private static bool CanControlService(string serviceName = null)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(serviceName))
                {
                    ...
                    if (serviceInfo == null)
                    {
                        Log.Warning("DBA Hawk service {serviceName} not found", serviceName);
                        return false;
                    }
                }
                else
                {
                    ...
                    if (serviceInfo == null)
                    {
                        Log.Warning("DBA Hawk service not found");
                        return false;
                    }
                }
        ...
        private static readonly string[] UpgradeProcessNames = ["DBAHawk", "DBADashServiceConfigTool", "DBAHawkConfig", "DBAHawkService"];

        /// <summary>
        /// Checks whether any DBA Hawk processes currently running from the service folder can be terminated
