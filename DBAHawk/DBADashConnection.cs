...
namespace DBAHawk
{
    public class DBADashConnection
    {
        ...
        private void ValidateSQLConnection()
        {
            ...
            {
                throw new Exception(
                    $"SQL Server Version {ConnectionInfo.MajorVersion} isn't supported by DBA Hawk.  For testing purposes, it's possible to skip this validation check.");
            }
            if (!supportedEngineEditions.Contains(ConnectionInfo.EngineEditionValue))
            {
                throw new Exception(
                    $"SQL Server Engine Edition {ConnectionInfo.EngineEditionValue} isn't supported by DBA Hawk.  For testing purposes, it's possible to skip this validation check.");
            }
        }
        ...
        private string GetDecryptedConnectionString(string _connectionString)
        {
            if (GetConnectionType(_connectionString) == ConnectionType.SQL)
            {
                ...
                {
                    builder.ApplicationName = "DBAHawk";
                }
                if (builder.Password.StartsWith(EncryptText.ObfuscationPrefix))
                {
