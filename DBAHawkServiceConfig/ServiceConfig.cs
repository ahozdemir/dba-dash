using DBAHawk;
using DBAHawk.InstanceMetadata;
...
using static DBAHawk.DBADashConnection;
using Control = System.Windows.Forms.Control;
using Font = System.Drawing.Font;
using SortOrder = System.Windows.Forms.SortOrder;

namespace DBAHawkServiceConfig
{
    public partial class ServiceConfig : Form, IThemedControl
    {
        ...
        private const string AiServiceName = "DBAHawkAI";
        ...
        private async Task ValidateDestinationAsync()
        {
            ...
            InvokeSetStatus(lblVersionInfo, "Validating...", HawkColors.HawkBlue, FontStyle.Regular);
            _ = Task.Run(async () =>
            {
        ...
        private async Task UpdateDBVersionStatusAsync()
        {
            ...
            if (collectionConfig.Destination == "")
            {
                ...
                InvokeSetStatus(lblVersionInfo, "Please start by setting the destination connection for your DBA Hawk repository database.", HawkColors.Information, FontStyle.Bold);
            ...
            {
                RepoDbVersionStatus = null;
                InvokeSetStatus(lblVersionInfo, "Invalid connection string, directory or S3 path", HawkColors.Fail, FontStyle.Regular);
                UpdateDeployDatabaseLinkState();
                return;
            }
            ...
            {
                RepoDbVersionStatus = null;
                InvokeSetStatus(lblVersionInfo, string.Empty, HawkColors.HawkBlue, FontStyle.Regular);
                UpdateDeployDatabaseLinkState();
                return;
            }
            ...
            try
            {
                ...
                {
                    case null:
                        InvokeSetStatus(lblVersionInfo, "???", HawkColors.Fail, FontStyle.Bold);
                        UpdateDeployDatabaseLinkState();
                        break;
                    ...
                    case DBValidations.DBVersionStatusEnum.CreateDB:
                        InvokeSetStatus(lblVersionInfo,
                            ...
                            HawkColors.Warning, FontStyle.Bold);
                        UpdateDeployDatabaseLinkState();
                        return;
                    ...
                    case DBValidations.DBVersionStatusEnum.OK:
                        InvokeSetStatus(lblVersionInfo,
                            ...
                            RepoDbVersionStatus.DACVersion, HawkColors.Success, FontStyle.Regular);
                        UpdateDeployDatabaseLinkState();
                        break;
            ...
            {
                InvokeSetStatus(lblVersionInfo, ex.Message, HawkColors.Fail, FontStyle.Regular);
                UpdateDeployDatabaseLinkState();
            }
        }
        ...
        private async void ServiceConfig_Load(object sender, EventArgs e)
        {
            ...
            dgvConnections.Columns.Add(new DataGridViewLinkColumn()
            {
                ...
                LinkColor = HawkColors.LinkColor
            });
            dgvConnections.Columns.Add(new DataGridViewLinkColumn()
            {
        ...
        private void SetConnectionCount()
        {
            ...
            if (cnt == 0)
            {
                ...
                lnkSourceConnections.LinkColor = HawkColors.Warning;
            }
            else
            {
                lnkSourceConnections.LinkColor = HawkColors.Success;
            }
            UpdateThreadCount();
        }
            ...
            try
            {
                ...
                this.Invoke(() =>
                {
                    if (service.Status.ToString() != lastStatus || lblServiceWarning.Text != warningText || runningAsText != lblRunningAs.Text)
                    {
                        ...
                        toolTip1.SetToolTip(lnkInstall, "Remove Windows service for DBA Hawk agent");
            ...
            catch (Exception ex)
            {
                ...
                if (lastStatus != status || lblServiceWarning.Text != warningText)
                {
                    this.Invoke(() =>
                    {
                        ...
                        toolTip1.SetToolTip(lnkInstall, "Install DBA Hawk agent as a Windows service");
        ...
        {
            if (MessageBox.Show("Are you sure you want to remove the DBA Hawk Windows service?", "Uninstall",
        ...
        private async Task<bool> SetDestinationAsync()
        {
            ...
            if (frm.DialogResult == DialogResult.OK)
            {
                ...
                {
                    builder.InitialCatalog = "DBAHawkDB";
                }
        ...
        {
            var psi = new ProcessStartInfo("https://github.com/ahmetozdemir/DBAHawk/blob/develop/Docs/Security.md")
                ...
                default:
                    {
                        ...
                        var page = new TaskDialogPage
                        {
                            ...
                            Text =
                                "💡A connection marked deleted can be restored using the recycle bin folder in the DBA Hawk GUI.",
            ...
            for (var i = e.RowIndex; i < e.RowIndex + e.RowCount; i++)
            {
                ...
                if (src.SourceConnection.Type == ConnectionType.SQL)
                {
                    ...
                    if (src.NoWMI)
                    {
                        ...
                        perfmonCell.LinkColor = HawkColors.NotApplicable;
                        perfmonCell.ActiveLinkColor = HawkColors.NotApplicable;
                    }
                    else
                    {
                        ...
                        perfmonCell.LinkColor = HawkColors.LinkColor;
                        perfmonCell.ActiveLinkColor = HawkColors.LinkColor;
                    }
                }
                var row = dgvConnections.Rows[i];
                if (hasStatus)
                {
                    var status = ConnectionStatuses.ContainsKey(src.ConnectionString) ? ConnectionStatuses[src.ConnectionString] : new("Unknown", HawkColors.NotApplicable, 2);
                ...
                {
                    value = "Skipped";
                    primaryBackColor = HawkColors.NotApplicable;
                }
                else if (SnapshotDates.TryGetValue(src.ConnectionID, out var snapshotDate))
                {
                    ...
                    primaryBackColor = age.TotalDays > 7 ? HawkColors.Fail :
                         age.TotalHours > 24 ? HawkColors.Warning : HawkColors.Success;
                }
                else
                {
                    value = "Not found";
                    primaryBackColor = HawkColors.Warning;
                }
            ...
            foreach (var row in dgvConnections.Rows.Cast<DataGridViewRow>())
            {
                async Task CheckConnectionAsync()
                {
                    ...
                    switch (src.SourceConnection.Type)
                    {
                        case ConnectionType.SQL:
                            try
                            {
                                ...
                                connectionStatus.TryAdd(src.ConnectionString,
                                    ("OK", HawkColors.Success, 0));
                            }
                            catch (Exception ex)
                            {
                                connectionStatus.TryAdd(src.ConnectionString,
                                    ($"Error: {ex.Message}", HawkColors.Fail, 99));
                            }
                        ...
                        default:
                            connectionStatus.TryAdd(src.ConnectionString,
                                ("N/A", HawkColors.NotApplicable, 1));
                            break;
                    }
                }
        ...
        {
            txtAllowScripts.Text = Enum.GetValues(typeof(ProcedureExecutionMessage.CommunityProcs)).Cast<DBAHawk.Messaging.ProcedureExecutionMessage.CommunityProcs>()
                .Select(c => c.ToString())
                .Aggregate((a, b) => a + "," + b);
        }
        ...
        {
            var isCustom = DBAHawk.PerformanceCounters.HasCustomPerformanceCounters();
        ...
        private async Task RefreshAiServiceStatusAsync()
        {
            ...
            try
            {
                ...
                this.Invoke(() =>
                {
                    ...
                    toolTip1.SetToolTip(lnkInstallAi, "Remove Windows service for DBAHawkAI API");
                });
            }
            catch
            {
                this.Invoke(() =>
                {
                    ...
                    toolTip1.SetToolTip(lnkInstallAi, "Install DBAHawkAI API as a Windows service");
                });
            }
        ...
        private string GetAiServiceDllPath() => Path.Combine(AppContext.BaseDirectory, "DBAHawkAI.dll");
        ...
        /// <c>dotnet DBAHawkAI.dll --check-runtime</c>, which exits immediately with
        ...
        private static void ShowRuntimeMissingDialog(string hostErrorMessage)
        {
            ...
            var page = new TaskDialogPage
            {
                ...
                Text = $"{detail}The DBAHawkAI service requires the ASP.NET Core Runtime 10.x (x64).\n\n{DotNet10DownloadUrl}",
                Icon = TaskDialogIcon.Warning,
        ...
        private async Task InstallAiServiceAsync()
        {
            ...
            {
                MessageBox.Show("DBAHawkAI.dll not found. Deploy DBAHawkAI next to this tool (DBAHawkAI\\DBAHawkAI.dll) or build DBAHawkAI first.", "AI Service", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            var frm = new InstallService
            {
                ...
                ServiceDescription = "DBA Hawk AI API service",
        ...
        private async Task UninstallAiServiceAsync()
        {
            ...
            if (MessageBox.Show("Remove DBAHawkAI Windows service?", "Uninstall AI Service", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
            {
                return;
            }
