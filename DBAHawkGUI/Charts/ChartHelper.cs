    ...
    internal class ChartHelper
    {
        ...
        private static Control GetPieChartFromDataTable(DataTable dt, PieChartConfiguration config)
        {
            ...
            var series = slices.Select(s => (ISeries)new PieSeries<ObservableValue>
            {
                ...
                DataLabelsPaint = config.DataLabelMode != PieLabelMode.None ? new SolidColorPaint(HawkColors.White.ToSKColor()) : null,
        ...
        /// <summary>
        /// Attach a standard context menu to chart controls providing Copy Image, Save Image As..., and optional Save Data actions.
        /// </summary>
        internal static void AddChartContextMenu(Control chart, DataTable dt = null)
        {
            ...
            try
            {
                ...
                if (dt != null)
                {
                    var dataItems = CreateDataMenuItems();
                    excel = dataItems.excel;
                    excelClick = dataItems.excelHandler;
                    copyData = dataItems.copyData;
                    copyDataClick = dataItems.copyDataHandler;
                    var sep = dataItems.sep;
        ...
        internal static CartesianChart GetChartFromDataTable(DataTable dt, ChartConfiguration config)
        {
            ...
            // Attach context menu so the returned chart is fully initialized
            AddChartContextMenu(chart, dt);
