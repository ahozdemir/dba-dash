...
namespace DBAHawkGUI.Changes
{
    internal class SQLPatchingView : CustomReportView
    {
        ...
        private async void TsUpdateBuildReference_Click(object sender, EventArgs e)
        {
            try
            {
                await DBAHawk.BuildReference.UpdateAsync(Common.ConnectionString);
                RefreshData();
        ...
        private void UpdateBuildReferenceStatus(DBADashDataGridView grid)
        {
                ...
                || drv["BuildReferenceUpdated"].DBNullToNull() is not DateTime updatedDate)
            {
                ...
                tsUpdateBuildReference.ForeColor = HawkColors.HawkBlue;
                return;
            }
            ...
            {
                tsUpdateBuildReference.ForeColor = buildReferenceDaysSinceUpdate <= Config.BuildReferenceUpdateExclusionPeriod ? HawkColors.HawkBlue : HawkColors.Fail;
            }
            else if (buildReferenceAge > Config.BuildReferenceAgeWarningThreshold)
            {
                tsUpdateBuildReference.ForeColor = buildReferenceDaysSinceUpdate <= Config.BuildReferenceUpdateExclusionPeriod ? HawkColors.HawkBlue : HawkColors.Warning;
            }
            else
            {
                tsUpdateBuildReference.ForeColor = HawkColors.Success;
            }
        }
            ...
            CustomReportResults = new Dictionary<int, CustomReportResult>
            {
                [0] = new CustomReportResult
                {
                    ...
                    Columns = new Dictionary<string, ColumnMetadata>
                    {
                        ...
                        ["PatchDate"] = new ColumnMetadata { DisplayIndex = 4, Alias = "SQL Patch Date", Description = "Date when DBA Hawk detected a version change" },
