...
namespace DBAHawkGUI.Changes
{
    public partial class InstanceMetadata : UserControl, ISetContext, IRefreshData
    {
        ...
        private static TreeNode CreateTreeNode(JToken token, string nodeName)
        {
            ...
            {
                ForeColor = DBADashUser.SelectedTheme.ThemeIdentifier == ThemeType.Dark ? HawkColors.White : HawkColors.HawkBlue
            };
            switch (token.Type)
            {
                ...
                default:
                    node.Nodes.Add(new TreeNode(token.ToString(Formatting.None)) { ForeColor = DBADashUser.SelectedTheme.ThemeIdentifier == ThemeType.Dark ? HawkColors.GreenPale : HawkColors.GreenDark });
                    break;
            }
            return node;
        }
