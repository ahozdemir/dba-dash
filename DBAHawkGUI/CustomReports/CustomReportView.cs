...
using DBAHawkGUI.Charts;
using DBAHawkGUI.CommunityTools;
    ...
    public partial class CustomReportView : UserControl, ISetContext, IRefreshData, ISetStatus, INavigation
    {
        ...
        public async Task RefreshDataRepository(CancellationTokenSource cts)
        {
            ...
            try
            {
                ...
                SetStatus("Running report", string.Empty, HawkColors.Information);
                ...
                if (ReferenceEquals(cts, cancellationTokenSource))
                    SetStatus("Completed", "Success", HawkColors.Success);
            }
            ...
            {
                SetStatus("Error running report:" + ex.Message, ex.ToString(), HawkColors.Fail);
            }
            finally
            {
                if (ReferenceEquals(cts, cancellationTokenSource))
        ...
        public void RefreshDataMessage()
        {
            ...
            {
                SetStatus("A message is already in progress.  Please wait for the current message to complete before running another.", "Warning", HawkColors.Warning);
                return;
            }
            var rpt = Report as DirectExecutionReport;
            if (rpt == null)
            {
                SetStatus("Invalid report type.  Expected a DirectExecutionReport", tooltip: null, HawkColors.Fail);
                return;
            }
            ...
            SetStatus("Message sent", "Running", HawkColors.Information);
            IsMessageInProgress = true;
        ...
        private Task ProcessCompletedMessage(ResponseMessage reply, Guid messageGroup, MessagingHelper.SetStatusDelegate setStatus)
        {
            ...
            {
                StopTimer();
                SetStatus(reply.Message, reply.Exception?.ToString(), HawkColors.Fail);
                IsMessageInProgress = false;
            }
            else if (reply.Type == ResponseMessage.ResponseTypes.Success)
            {
                StopTimer();
                SetStatus(reply.Message, null, HawkColors.Success);
                IsMessageInProgress = false;
            }
            else if (reply.Type == ResponseMessage.ResponseTypes.Progress)
            {
                SetStatus(reply.Message, null, HawkColors.Information);
            }
        ...
        private async Task CancelProcessing()
        {
            ...
            if (Report is DirectExecutionReport)
            {
                if (CurrentMessageGroup != Guid.Empty)
                {
                    ...
                    SetStatus("Cancellation requested", "", HawkColors.Warning);
                    await MessagingHelper.SendMessageAndProcessReply(msg, context, SetStatus, ProcessCompletedMessage,
                        CurrentMessageGroup);
                }
                else
                {
                    SetStatus("Nothing to cancel", "", HawkColors.Information);
                    IsMessageInProgress = false;
                }
            }
            else
