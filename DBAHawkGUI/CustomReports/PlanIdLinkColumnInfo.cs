...
namespace DBAHawkGUI.CustomReports
{
    internal class PlanIdLinkColumnInfo : LinkColumnInfo
    {
        ...
        public override async void Navigate(DBADashContext context, DataGridViewRow row, int selectedTableIndex, ContainerControl sender)
        {
            ...
            {
                status?.SetStatus($"Column '{PlanIdColumn}' not found", string.Empty, HawkColors.Fail);
                return;
            }
            ...
            if (planId == null)
            {
                status?.SetStatus("Plan ID is null", string.Empty, HawkColors.Fail);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(DatabaseNameColumn))
            {
                ...
                {
                    status?.SetStatus($"Column '{DatabaseNameColumn}' not found", string.Empty, HawkColors.Fail);
                    return;
                }
            ...
            if (db == null)
            {
                status?.SetStatus("Database name is null", string.Empty, HawkColors.Fail);
                return;
            }
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    status?.SetStatus($"Error collecting plan: {ex.Message}", ex.ToString(), HawkColors.Fail);
                }
            });
        }
        ...
        private static Task ProcessCompletedPlanCollectionMessage(ResponseMessage reply, Guid messageGroup, MessagingHelper.SetStatusDelegate setStatus)
        {
            ...
            {
                setStatus?.Invoke("No data returned", string.Empty, HawkColors.Warning);
        ...
        private static void LoadQueryPlan(DataTable dt, MessagingHelper.SetStatusDelegate setStatus)
        {
            if (dt.Rows.Count == 0)
            {
                setStatus?.Invoke("No query plan", string.Empty, HawkColors.Fail);
                return;
            }

            if (!dt.Columns.Contains("query_plan"))
            {
                setStatus?.Invoke("Response missing 'query_plan' column", string.Empty, HawkColors.Fail);
                return;
            }
            ...
            {
                setStatus?.Invoke("Query plan is empty", string.Empty, HawkColors.Fail);
                return;
            }
            ...
            setStatus?.Invoke("Loading Query Plan...", string.Empty, HawkColors.Success);

            try
            {
                Common.ShowQueryPlan(plan, fileName);
                setStatus?.Invoke("Query plan loaded in associated app", string.Empty, HawkColors.Success);
            }
            catch (Exception ex)
            {
                setStatus?.Invoke($"Failed to load query plan: {ex.Message}", ex.ToString(), HawkColors.Fail);
            }
        }
    }
}