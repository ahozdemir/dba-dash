...
namespace DBAHawkGUI.Messaging
{
    internal static class CollectionMessaging
    {
            ...
            ISetStatus control, string db = null)
        {
            ...
            {
                control.SetStatus("Too many pending requests", null, HawkColors.Fail);
                return;
            }
            ...
            foreach (var type in types)
            {
                ...
                {
                    control.SetStatus($"Collection {type} already triggered recently for {key}", null, HawkColors.Fail);
                    return;
                }
                else if (type is "QueryText" or "QueryPlans")
                {
                    control.SetStatus($"Collection {type} is collected as part of RunningQueries collection and can't be triggered manually", null, HawkColors.Warning);
                    return;
                }
            }
        ...
        private static async Task SendAndReceiveReply(CollectionMessage message, int importAgentID, string key, string messageBase, ISetStatus control)
        {
            ...
            try
            {
                ...
                control.SetStatus(messageBase + "SENT", "", HawkColors.Information);
                ...
                while (!completed)
                {
                    ...
                    switch (reply.Type)
                    {
                        case ResponseMessage.ResponseTypes.Failure:
                            control.SetStatus(messageBase + reply.Message, reply.Exception?.ToString(), HawkColors.Fail);
                            completed = true;
                            break;
                        ...
                        case ResponseMessage.ResponseTypes.Warning:
                                ...
                                PromptRunUnscheduled(control, reply.DisabledCollections))
                            {
                                ...
                                await MessageProcessing.SendMessageToService(message.Serialize(), importAgentID, group, Common.ConnectionString, message.Lifetime);
                                control.SetStatus(messageBase + "running unscheduled collection(s)...", null, HawkColors.Information);
                            ...
                            control.SetStatus(messageBase + reply.Message, null, HawkColors.Warning);
                            break;
                        ...
                        case ResponseMessage.ResponseTypes.Progress:
                            control.SetStatus(messageBase + reply.Message, null, HawkColors.Information);
                            break;

                        case ResponseMessage.ResponseTypes.Success:
                            control.SetStatus(messageBase + reply.Message, null, HawkColors.Success);
                            control.RefreshData();
                            break;
                    }
                }
