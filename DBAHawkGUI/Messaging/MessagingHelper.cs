...
namespace DBAHawkGUI.Messaging
{
    public class MessagingHelper
    {
            ...
            SetStatusDelegate setStatus, MessageCompletedDelegate processCompleted, Guid messageGroup)
        {
            ...
            {
                setStatus("No Import Agent", string.Empty, HawkColors.Fail);
                return;
            }
            ...
            MessageResponseDelegate onProgress = null)
        {
            ...
            {
                setStatus(ex.Message, ex.ToString(), HawkColors.Fail);
                return;
            }
            ...
            while (!completed)
            {
                ...
                {
                    setStatus(ex.Message, string.Empty, HawkColors.Fail);
                    completed = true;
                    return;
                }
                ...
                switch (reply.Type)
                {
                    case ResponseMessage.ResponseTypes.Progress:
                        completed = false;
                        setStatus(reply.Message, string.Empty, HawkColors.Information);
                        if (onProgress != null)
                        {
                    ...
                    case ResponseMessage.ResponseTypes.Warning:
                        ...
                        setStatus(reply.Message, null, HawkColors.Warning);
                        await processCompleted(reply, messageGroup, setStatus);
                        break;
