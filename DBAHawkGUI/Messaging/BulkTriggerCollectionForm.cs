...
namespace DBAHawkGUI.Messaging
{
    ...
    public partial class BulkTriggerCollectionForm : Form
    {
        ...
        private void Grid_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            {
                "Success" => HawkColors.Success,
                "Failed" => HawkColors.Fail,
                "Warning" => HawkColors.Warning,
                "Cancelled" => HawkColors.Warning,
                "Pending" => HawkColors.Information,
        ...
        private async Task RunBatchAsync(List<TriggerInstance> batchInstances, bool force)
        {
            ...
            {
                SetStatus("Error: " + ex.Message, HawkColors.Fail);
            }
            finally
            {
        ...
        private async Task SendGroupAsync(ActiveGroup group, List<TriggerInstance> batchInstances)
        {
            ...
            try
            {
                await MessagingHelper.SendMessageAndProcessReply(message, group.ImportAgentID,
                    (msg, _, color) => { if (color == HawkColors.Fail) sendError = msg; },
        ...
        private async Task RequestCancellationAsync()
        {
            ...
            {
                SetStatus("Error requesting cancellation: " + ex.Message, HawkColors.Fail);
            }
        }
        ...
        private void UpdateSummary() => InvokeOnUI(() =>
        {
            ...
            if (!done)
            {
                ...
                color = (failed > 0 || cancelled > 0) ? HawkColors.Warning : HawkColors.Information;
            }
            else if (failed == 0 && cancelled == 0 && warning == 0)
            {
                color = HawkColors.Success; // everything succeeded
            }
            else if (succeeded > 0 || warning > 0 || cancelled > 0)
            {
                color = HawkColors.Warning; // partial - some succeeded / skipped / cancelled, but not all failed
            }
            else
            {
                color = HawkColors.Fail; // everything failed
            }
