...
* ### DBAHawkDB
    The DBA Hawk repository database source code.  The project output is a *.dacpac file that is used to create and upgrade the repository database.
...
* ### ServiceConfigTool
    A Windows Forms app that is used to manage the ServiceConfig.json file.  This serves the same purpose as the *DBAHawkConfig* project but in GUI instead of commandline format.  e.g. Adding new SQL Server connections to the config.

* ### DBADashReports
    SSRS reports for displaying the collected data.  This is a legacy project that has been replaced with *DBAHawkGUI*, but this could be revived with community contributions.
...
The code for existing collections is located [DBAHawk/SQL](DBAHawk/SQL).  If you need to add or remove columns from the results you will need to update the associated user defined type in the DBAHawkDB project.  Also, the stored procedure that handles the import called *{CollectionType}_Upd* will also need to be modified along with the tables that store the data.  In the DBImporter.cs file there is a UpgradeDS method that should be modified to add the extra columns to the DataTable if they don't already exist.  The call to UpgradeDS is to allow us to import data collected by older versions of DBA Hawk. e.g. If you have a remote instance of DBA Hawk that pushes data to a S3 bucket you don't need to upgrade this instance at the same time.

## Adding a new collection

DBA Hawk can already collect [custom performance counters](OSPerformanceCounters.md) and you can add also add [custom checks](CustomChecks.md).  If those options are not suitable you might consider modifying DBA Hawk to add a new collection.  Ideally the changes you make will be contributed back to the main project as other people will be able to benefit from them and it will make updates easier for you in future.  To be included back into the main project the changes must have value to other users of DBA Hawk - consider how you can make the changes generic and avoid including any bespoke features.  If you still need to develop bespoke features and functionality these will need to be maintained in a separate fork of the project.

* Create a SQL Query that will capture the data for the new collection. 
    - Consider the impact of collecting this data and the frequency of collection.  The query should run on all versions of SQL Server supported by DBA Hawk or code added to exclude the collection from unsupported versions
* Add a new item to the DBAHawk project under the SQL folder.  Select "Text" file and name the file "SQL{MyCollection}.sql"
...
* The DBAHawkDB project will need to be modified to add:
