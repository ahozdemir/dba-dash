...
BEGIN
	...
	/* Check if we have any DBA Hawk alert rules that might apply */
	IF EXISTS(	SELECT 1 
				FROM Alert.Rules 
				WHERE Type = @Type 
				AND IsActive=1
	...
	BEGIN
		...
		IF @@ROWCOUNT >0
		BEGIN
			/* Filter the new sql agent alerts based on the DBA Hawk alert rules */
	...
	IF @AlertDetailCount >0 /* Update DBA Hawk alerts if needed */
	BEGIN
