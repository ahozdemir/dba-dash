/*
	Migrates dbo.DBFileSnapshot history from one instance to another within the same DBAHawk repository database.
	Use when a database or server has been moved/replaced and you want to preserve historical file size/space metrics.

	PARAMETERS:
		@SourceRepositoryDatabase - The DBAHawk repository database containing source history (defaults to current database)
	...
	EXAMPLE 3 - Migrate from a different DBAHawk repository database:
		EXEC Migration.DBFileSnapshot
