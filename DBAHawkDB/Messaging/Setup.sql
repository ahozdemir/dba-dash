...
BEGIN
	...
	SET @SQL = 'IF NOT EXISTS(SELECT 1 FROM sys.service_message_types WHERE name = ''//dbadash.com/DBAHawkService/Send'')
	BEGIN
		CREATE MESSAGE TYPE [//dbadash.com/DBAHawkService/Send];
	END
	IF NOT EXISTS(SELECT 1 FROM sys.service_message_types WHERE name = ''//dbadash.com/DBAHawkService/Reply'')
	BEGIN
		CREATE MESSAGE TYPE [//dbadash.com/DBAHawkService/Reply];
	END
	IF NOT EXISTS(SELECT 1 FROM sys.service_contracts WHERE name = ''//dbadash.com/DBAHawkService/ComsContract'')
	BEGIN
		CREATE CONTRACT [//dbadash.com/DBAHawkService/ComsContract]           
			( [//dbadash.com/DBAHawkService/Send]  SENT BY INITIATOR,           
			  [//dbadash.com/DBAHawkService/Reply] SENT BY TARGET  
			) ;  
	END
	...
	IF NOT EXISTS(SELECT 1 FROM sys.services WHERE name = ''//dbadash.com/DBAHawkService/ComsInit'')
	BEGIN
		CREATE SERVICE [//dbadash.com/DBAHawkService/ComsInit]
	...
	*/
	...
	END
	IF NOT EXISTS(SELECT 1 FROM sys.services WHERE name = ''//dbadash.com/DBAHawkService/ComsTarget',@DBADashAgentID ,''')
	BEGIN
		CREATE SERVICE [//dbadash.com/DBAHawkService/ComsTarget',@DBADashAgentID ,']
				ON QUEUE DBADashServiceTargetQueue',@DBADashAgentID,'
				([//dbadash.com/DBAHawkService/ComsContract])
...
ELSE
BEGIN
	SET @SQL = CONCAT('IF EXISTS(SELECT 1 FROM sys.services WHERE name = ''//dbadash.com/DBAHawkService/ComsTarget',@DBADashAgentID ,''')
	BEGIN
		DROP SERVICE [//dbadash.com/DBAHawkService/ComsTarget',@DBADashAgentID ,']
	END
