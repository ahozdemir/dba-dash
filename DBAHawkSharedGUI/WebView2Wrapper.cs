...
namespace DBAHawkGUI.AgentJobs
{
    public partial class WebView2Wrapper : UserControl
    {
        ...
        {
            InitializeComponent();
            lnkDownload.LinkColor = HawkColors.LinkColor;
            lblNotice.ForeColor = HawkColors.Fail;
        }
        ...
        private void LoadHTMLFromDisk(Exception ex, string html)
        {
            ...
            catch (Exception ex2)
            {
                WebView2.NavigateToString(
                    $"<html><body style='background-color:{HawkColors.Fail.ToHexString()};color:#ffffff'>Error loading HTML:<br/>{HttpUtility.HtmlEncode(ex.ToString()) + "<br/>" + HttpUtility.HtmlEncode(ex2.ToString())}</body></html>");
            }
        }
