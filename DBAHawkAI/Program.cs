...
using DBAHawk;
using DBAHawkAI.Configuration;
using DBAHawkAI.Models;
using System.Text.Json;
using DBAHawkAI.Services;
using DBAHawkAI.Services.Authentication;
using DBAHawkAI.Services.Tools;
using Serilog;
...
Log.Logger = new LoggerConfiguration()
    .ReadFrom.Configuration(
        new ConfigurationBuilder()
            ...
            .AddJsonFile("serilog.DBAHawkAI.json", optional: false, reloadOnChange: true)
            .Build())
    .Enrich.FromLogContext()
    .Enrich.WithProperty("ApplicationName", "DBAHawkAI")
    .Enrich.WithProperty("MachineName", Environment.MachineName)
    .CreateLogger();

try
{
    Log.Information("DBAHawkAI starting up");
    RunApp(args);
    Log.Information("DBAHawkAI shut down cleanly");
}
catch (Exception ex)
{
    Log.Fatal(ex, "DBAHawkAI terminated unexpectedly");
}
finally
{
...
var cachedRepositoryFingerprint = DBAHawk.RepositoryFingerprint.GetFingerprint(repositoryConnectionString);
...
app.MapGet("/api/ai/health", () =>
{
    if (cachedRepositoryFingerprint == null)
    {
        ...
        {
            cachedRepositoryFingerprint ??= DBAHawk.RepositoryFingerprint.GetFingerprint(repositoryConnectionString);
        }
    }
