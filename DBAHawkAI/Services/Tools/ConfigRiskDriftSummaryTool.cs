...
namespace DBAHawkAI.Services.Tools
{
    public class ConfigRiskDriftSummaryTool : IAiTool
    {
        private readonly SqlToolExecutor _sql;

        public ConfigRiskDriftSummaryTool(SqlToolExecutor sql)
        {
            _sql = sql;
        }
        ...
        public string[] Keywords => ["drift", "config risk", "configuration change", "trace flag", "patch", "patching", "changed", "change risk", "risky change", "high risk config", "dangerous setting", "review change", "audit", "compliance", "baseline", "deviation", "hardware change", "driver change", "cu", "cumulative update", "service pack"];

        public async Task<AiToolResult> RunAsync(AiAskRequest request, CancellationToken cancellationToken)
        {
            var rows = await _sql.QueryAsync("AI.ConfigRiskDrift_Get", request.MaxRows, request.InstanceFilter, request.HoursBack, cancellationToken);
