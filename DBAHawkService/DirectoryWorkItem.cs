...
namespace DBAHawkService
{
    public sealed class DirectoryWorkItem : IWorkItem
    {
        ...
        /// <summary>
        /// Process a given list of files in order for a specific instance, writing collected data to the DBAHawk repository database
        /// </summary>
        ...
        internal static string GetID(DataSet ds)
        {
            try
            {
                return ds.Tables["DBAHawk"].Rows[0]["Instance"] + "_" + ds.Tables["DBAHawk"].Rows[0]["DBName"];
            }
            catch (Exception ex)
            {
