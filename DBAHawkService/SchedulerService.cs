            ...
            await pipeline.ExecuteAsync(async _ =>
            {
                try
                {
                    ...
                    do
                    {
                        ...
                        {
                            Log.Warning(
                                "Timeout waiting for DB deployment to complete.  It's possible that a previous DB deployment was interrupted or is still running but taking longer than expected on another DBA Hawk service instance.");
            ...
            if (src.SourceConnection.Type == ConnectionType.SQL && !OfflineInstances.IsOffline(src))
            {
                try
                {
                    if (src.SourceConnection.ConnectionInfo.IsXESupported)
                    {
                        ...
                        {
                            Log.Logger.Information("Stop DBAHawk event sessions for {connection}", src.SourceConnection.ConnectionForPrint);
                            await collector.StopEventSessionsAsync();
                        }
                        else
                        {
                            Log.Logger.Information("Remove DBAHawk event sessions for {connection}", src.SourceConnection.ConnectionForPrint);
                ...
                {
                    Log.Logger.Error(ex, "Error Stop/Remove DBAHawk event sessions for {connection}", src.SourceConnection.ConnectionForPrint);
                }
            }
        }
